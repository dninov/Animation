document.addEventListener("DOMContentLoaded", function(event) { 
    ...
    window.onscroll = function() {
        ...
        if (window.scrollY > 0 && logoBig == true) {
           ...
           gsap.to(topBar, {duration:1, y: 80, ease:Power1.easeOut});
            ...
            gsap.to(logoTxt, {delay:0.5, duration:1, y: 0, ease:Power3.easeOut})
    ...
    function logoSmall(){
        ...
        if (window.innerWidth < 600) {
            console.log("<600");
            gsap.set(logo, { scale:0.4, y:"-50%",left:"25%", zIndex:50})
        }
        else if (window.innerWidth < 800){
            console.log("<800");
            gsap.set(logo, { scale:0.3, y:"-50%",left:"15%", zIndex:50})
        }
        else if (window.innerWidth < 1200){
            console.log("<1200");
            gsap.set(logo, { scale:0.2, y:"-50%",left:"10%", zIndex:50})
        }else{
            console.log("else");
            gsap.set(logo, { scale:0.15, y:"-46%",left:"8%", zIndex:50})
        }
      setSmallLogo();
    }
    function setSmallLogo(){
        ...
        if (window.innerWidth < 600) {
            console.log("<600");
            gsap.to(logo, { scale:0.4, y:"-37%",left:"25%", zIndex:50})
        }
        else if (window.innerWidth < 800){
            console.log("<800");
            gsap.to(logo, { scale:0.3, y:"-42%",left:"15%", zIndex:50})
        }
        else if (window.innerWidth < 1200){
            console.log("<1200");
            gsap.to(logo, { scale:0.2, y:"-45%",left:"10%", zIndex:50})
        }else{
            console.log("else");
            gsap.to(logo, { scale:0.15, y:"-46%",left:"8%", zIndex:50})
        }
    }
    function Resize(){
        if (logoBig == false) {
